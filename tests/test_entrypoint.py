...
class EntrypointTests(unittest.TestCase):
    def setUp(self):
        ...
        self.stack.enter_context(patch.multiple(
            ...
            HEALTH_CHECK_TIMEOUT='5', HEALTH_CHECK_MAX_RETRY='3', HOST_IP_SWAP_MAX_RETRY='3'))
        self.host = MagicMock()
        self.host.get_current_ip.return_value = '192.0.2.1'
        self.host.swap_ip.return_value = ('192.0.2.1', {})
        self.stack.enter_context(patch.object(index, 'LightsailHelper', return_value=self.host))
        self.dns = MagicMock()
        self.dns.get_current_ip.return_value = '192.0.2.1'
        self.stack.enter_context(patch.object(index, 'Route53Helper', return_value=self.dns))
    ...
    def test_dict_string_and_bytes_events_accept_boolean_force(self):
        for force in [True, False, 'true', 'false']:
            for encode in [lambda value: value, json.dumps,
                ...
                with self.subTest(force=force, encode=encode):
                    ...
                    self.assertEqual(result, {'dns': 'host.example.com', 'ip': '192.0.2.1'})
                    self.assertEqual(self.host.swap_ip.call_count,
                                     1 if force is True or force == 'true' else 0)
    ...
    def test_nonpositive_numeric_settings_use_working_defaults(self):
        with patch.multiple(index, HEALTH_CHECK_TIMEOUT='-1',
            ...
            self.assertEqual(index.main_handler({'force_swap': True}, None)['ip'], '192.0.2.1')
    ...
    def test_cli_healthy_host_exits_without_swapping(self):
        ...
        self.assertEqual(json.loads(output.getvalue().splitlines()[-1]),
                         {'dns': 'host.example.com', 'ip': '192.0.2.1'})
    ...
    def test_cli_force_replaces_healthy_ip_and_updates_dns(self):
        ...
        output = io.StringIO()
        with redirect_stdout(output):
            status = index.main(['--force-swap'])
        self.assertEqual(status, 0)
        self.assertEqual(json.loads(output.getvalue().splitlines()[-1])['ip'], '192.0.2.2')
